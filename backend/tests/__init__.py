...
from backend.src.numbers_solver.numbers_backtracking import NumbersSolver
from backend.src.server import ServiceException, ServiceCodes
from backend.src.server.NumbersService import NumberService
from backend.src.server.flask_inits import createApp, celery
from backend.src.server.flask_server import app
from backend.src.server.flask_socketio_server import socketio
...
class testCeleryTask(unittest.TestCase):
    def setUp(self):
        celery.config_from_object('backend.src.server.config.TestConfig')
        self._celery_task = CeleryTask()

    @mock.patch('backend.src.server.tasks.NumberService.solve', autospec=True)
    def testRun(self, mock_class):
        with mock.patch('backend.src.server.tasks.CeleryTask.update_state'):
...
class testFlaskServer(unittest.TestCase):
    def setUp(self):
        app.config.from_object('backend.src.server.config.TestConfig')
        celery.config_from_object('backend.src.server.config.TestConfig')
    ...
    @mock.patch('backend.src.server.sync_api.NumberService.solve', autospec=True)
    ...
    @mock.patch('backend.src.server.flask_server.CeleryTask.delay', autospec=True)
    ...
    @mock.patch('backend.src.server.flask_server.CeleryTask.AsyncResult', autospec=True)
    def testAsyncDataOk(self, mock_class):
...
class testSocketIOServer(unittest.TestCase):
    def setUp(self):
        self._flask_app = createApp('backend.src.server.config.TestConfig', socket_io=True)
